...
import StatusChangeModal from './StatusChangeModal'; // Import the explanation modal
...
const ComplaintDetailModal = ({ complaint, onClose, onUpdate }) => {
  ...
  const [pendingStatusChange, setPendingStatusChange] = useState(null); // NEW: Handles explanation popup
  ...
  // NEW: Intercepts status changes that require an explanation
  const handleFieldChange = async (field, value) => {
    if (field === 'status' && ['IN_PROGRESS', 'RESOLVED', 'REJECTED'].includes(value)) {
      setPendingStatusChange(value); // Opens the explanation popup
      return;
    }
  ...
  // NEW: Confirms the status change with the explanation text
  ...
  return (
    <>
            ...
            {activeTab === 'details' && (
              <div className="space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="flex flex-col gap-1"><span className="text-xs uppercase font-bold text-base-content/50 tracking-wider">Date</span><span className="text-sm font-medium">{formatDate(complaint.updated_at)}</span></div>
                </div>
      ...
      {/* Render Explanation Popup on top of the Detail Modal */}
      {pendingStatusChange && (
        <StatusChangeModal 
