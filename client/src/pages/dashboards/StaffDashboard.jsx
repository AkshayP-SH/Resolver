...
export default function StaffDashboard() {
  ...
  const renderPage = () => {
    switch (currentPage) {
      case 'overview': return <StaffOverview complaints={complaints} assignedToMe={assignedToMe} user={user} onSelectComplaint={setSelectedComplaint} />;
      case 'all-complaints': return <AllComplaintsView onAssign={handleAssignToMe} onSelectComplaint={setSelectedComplaint} />;
      case 'assigned-to-me': return <AssignedToMeView complaints={assignedToMe} loading={loading} onStatusChange={handleStatusChange} onSelectComplaint={setSelectedComplaint} />;
      case 'new-complaint': return <NewComplaintForm onCreated={fetchComplaints} />;
      default: return <StaffOverview complaints={complaints} assignedToMe={assignedToMe} user={user} onSelectComplaint={setSelectedComplaint} />;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-base-200 md:pl-16 transition-all duration-300">
      ...
      {selectedComplaint && (
        <ComplaintDetailModal complaint={selectedComplaint} onClose={() => setSelectedComplaint(null)} onUpdate={fetchComplaints} />
      )}
    </div>
  );
}
...
function AssignedToMeView({ complaints, loading, onSelectComplaint }) {
  const [filters, setFilters] = useState({});
  ...
  return (
    <div>
      ...
      <div className="card bg-base-100 shadow-sm rounded-none border border-base-300">
        <div className="card-body p-0">
          {complaints.length === 0 ? (
            <p className="text-center py-12 text-base-content/50 font-semibold">No complaints assigned to you yet.</p>
          ) : filtered.length === 0 ? (
